...
export function WhatYouGetClient({ columns }: { columns: Column[] }) {
  ...
  return (
    <section id="what-you-get" className="px-4 py-10 md:py-14">
      <div className="w-full">
        ...
        <div className="relative w-full rounded-[28px] md:rounded-[32px] border border-white/10 bg-black/40 backdrop-blur-xl overflow-hidden shadow-[0_10px_40px_-10px_rgba(0,0,0,0.6)] py-[160px] md:py-[180px]">
          ...
          <div className="absolute top-6 left-4 md:top-8 md:left-12 text-base md:text-lg tracking-wide text-white/80 z-10">
            <span className="font-medium">What </span>
            <span className="font-bold text-white">You Get</span>
          </div>

          {/* Centered content */}
          <div className="relative z-10 grid lg:grid-cols-2 gap-6 md:gap-8 items-center px-4 md:px-12">
            {/* Left column */}
              ...
              <ul className="space-y-3 text-white/90">
                {left?.bullets?.map((b, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <svg className="mt-1.5 h-4 w-4 flex-shrink-0 text-white/80" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
